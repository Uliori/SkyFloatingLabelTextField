...
open class SkyFloatingLabelTextField: UITextField {
    ...
    @IBInspectable open var labelPadding:CGFloat = 0
    
    /// A value that determines the padding of the bottom line
    @IBInspectable open var linePadding:CGFloat = 0{
        didSet {
            self.updateLineView()
        }
    }
    ...
    open func titleLabelRectForBounds(_ bounds:CGRect, editing:Bool) -> CGRect {
        let titleHeight = self.titleHeight()
        if editing {
            return CGRect(x: labelPadding, y: 0, width: bounds.size.width, height: titleHeight)
        }
        return CGRect(x: labelPadding, y: titleHeight, width: bounds.size.width, height: titleHeight)
    }
    ...
    open func lineViewRectForBounds(_ bounds:CGRect, editing:Bool) -> CGRect {
        ...
        return CGRect(x: self.linePadding, y: bounds.size.height - lineHeight, width: bounds.size.width, height: lineHeight);
    }
...
open class RLSkyFloatingLabelTextField: SkyFloatingLabelTextField {
    
    open var shouldDisplayRightView: Bool {
        return self.rightViewMode == .always
            || (self.isEditing && self.rightViewMode == .whileEditing)
            || (!self.isEditing && self.rightViewMode == .unlessEditing)
    }
